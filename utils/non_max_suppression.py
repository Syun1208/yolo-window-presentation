...
def nms(bounding_boxes, confidence_score, threshold=0.4):
    ...
    start_x = boxes[:, 0]
    start_x = np.array([int(i) for i in start_x])
    start_y = boxes[:, 1]
    start_y = np.array([int(i) for i in start_y])
    end_x = boxes[:, 2]
    end_x = np.array([int(i) for i in end_x])
    end_y = boxes[:, 3]
    end_y = np.array([int(i) for i in end_y])
...
if __name__ == '__main__':
    image_name = '/home/long/Downloads/datasets/datasetsYOLO/images/train/' \
                 '20210723_093523_image_jpeg.rf.58baf45800b803df8472cca1c7c1e556.jpg'
